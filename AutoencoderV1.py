...
autoencoder.compile(optimizer='adam', loss='mean_absolute_error')  # binary_crossentropy
...
# autoencoder.save("models/autoencoderV2.h5")

print("Model Saved")    # To load: tf.keras.models.load_model(model_path)

autoencoder = load_model("models/autoencoderV2.h5")
...
ANOMALIES = np.where(abs_diff > error_SD)[INDEX_TO_CHECK]
...
plt.plot(X_val[:ANOMALIES[100], INDEX_TO_CHECK], label="x")
plt.plot(Y_val[:ANOMALIES[100], INDEX_TO_CHECK], label="y")
plt.plot(ANOMALIES[:100], X_val[ANOMALIES[:100], INDEX_TO_CHECK], 'ro', label='Difference > Threshold')
